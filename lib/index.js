...
let Plug = (_dec = (0, _neovim.Plugin)({ dev: false }), _dec2 = (0, _neovim.Function)('PlugUpdate', { sync: false }), _dec3 = (0, _neovim.Function)('PlugRetry', { sync: false }), _dec4 = (0, _neovim.Command)('PlugCheck', {
  sync: true
  ...
  async plugCheck() {
    ...
    for (let plug of plugins) {
      ...
      try {
        let stat = await pify(_fs2.default.stat)(dir);
        if (!stat.isDirectory()) {
          await this.nvim.command(`echoerr '${dir} not a directory!'`);
        } else {
          dirs.push(_path2.default.basename(dir));
        }
      } catch (e) {
    ...
    let baseDir = _path2.default.join(process.env.HOME, '.vim/bundle');
    let files = await pify(_fs2.default.readdir)(baseDir);
    for (let file of files) {
      let dir = _path2.default.join(baseDir, file);
      let stat = await pify(_fs2.default.stat)(dir);
